...
# ---------------------------------------------------------------------------
# OpenCode CLI invocation  (verified against opencode 1.18.32)
# ---------------------------------------------------------------------------


_OPENCODE_AGENT_PREFIX = "skillspector-deny-all"
_OPENCODE_DENY_ALL = json.dumps({"*": "deny"}, separators=(",", ":"))
_OPENCODE_SUPPORTED_VERSION = "1.18.32"
    ...
    env.update(
        {
            ...
            "OPENCODE_PURE": "1",
            # OpenCode 1.18.32 uses this internal override for the home paths
...
def _preflight_opencode_policy(
    ...
    not have a supported disable flag in OpenCode 1.18.32. Querying the final
...
def _build_opencode_argv(binary: str, model: str, max_output_tokens: int = 0) -> list[str]:
    ...
    Flags chosen (verified against ``opencode`` 1.18.32 ``run --help``):
...
def _parse_opencode_output(raw: str) -> str:
    ...
    Verified against opencode 1.18.32, whose events look like::
