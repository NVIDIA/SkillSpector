...
def analyze(
    ...
    *,
    ...
    classification_context_by_line: dict[int, str] = {}
    ...
    def classification_ctx(start: int) -> str:
        """Retain TM1's legacy three-line classification scope privately."""
        line = line_number(start)
        context = classification_context_by_line.get(line)
        if context is None:
            line_index = line - 1
            context = "\n".join(
                content_lines[max(0, line_index - 3) : min(len(content_lines), line_index + 4)]
            )
            classification_context_by_line[line] = context
        return context
    ...
    for (
        ...
        classification_context = classification_ctx(match_start)
        context_text = (
