...
def _uv_arguments_execute_appended_script(
    ...
    *,
    ...
    can receive it and load it from argv.  Exact terminal version queries and
    bare launchers whose appended source cannot select ``run`` or an option are
    certified as non-executing.
    """
    if any(argument.dynamic_offsets for argument in arguments):
        return None
    if not arguments:
        source_basename = source_path.rsplit("/", 1)[-1]
        source_selects_run = source_basename == "run" or (
            allow_filesystem_aliases
            and unicodedata.normalize("NFD", source_basename).casefold() == "run"
        )
        if source_selects_run or _implicit_python_source_path_can_be_an_option(source_path):
            # The kernel-appended source can become the ``run`` subcommand, or
            # a global option that leaves caller-supplied argv to select it.
            # Later arguments can then choose this artifact with ``--script``.
            return None
        return False
...
def _record_source_execution(
    ...
    *,
    ...
    if is_python_interpreter:
        ...
        executes_script = _uv_arguments_execute_appended_script(
            execution.arguments,
            source_path,
            allow_filesystem_aliases=allow_filesystem_aliases,
        )
