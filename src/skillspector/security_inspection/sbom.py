...
def _parse_requirements(skill_path: Path) -> list[dict[str, str]]:
    ...
    req_files = [
        ...
        skill_path / "requirements.pip",
        skill_path / "pyproject.toml",
        skill_path / "setup.py",
        skill_path / "Pipfile",
        skill_path / "environment.yml",
    ]
    for rf in req_files:
        if not rf.exists():
            continue
            ...
            if rf.name == "requirements.txt":
                for line in text.splitlines():
                    ...
                    if m:
                        ...
                        components.append(
                            {"name": name, "version": ver or "unknown", "source": str(rf.name)}
                        )
                    ...
                    for dep in re.findall(r'"([^"]+)"', deps_match.group(1)):
                        ...
                        if m2:
                            components.append(
                                {
                                    "name": m2.group(1),
                                    "version": m2.group(2).strip(" =<>~!") or "unknown",
...
def generate_sbom(skill_path: Path, skills_root: Path) -> dict[str, Any]:
    ...
    for p in skill_path.rglob("*"):
        if (
            p.is_file()
            and p.suffix in (".py", ".json", ".yaml", ".yml", ".txt", ".toml", ".cfg", ".md")
    ...
    components.append(
        {
            ...
            "hashes": [
                {"alg": "SHA-256", "content": hashlib.sha256(skill_name.encode()).hexdigest()}
            ],
    ...
    bom = {
        ...
        "metadata": {
            ...
            "tools": [
                {"vendor": "NVIDIA", "name": "Skill Inspector Security Plugin", "version": "1.0.0"}
            ],
        },
