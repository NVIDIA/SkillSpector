...
CAPABILITY_PATTERNS = {
    "file_access": {
        "read": [r"\bopen\s*\(", r"\bPath\s*\(", r"os\.path", r"pathlib", r"read_text", r"read_bytes", r"glob\.", r"rglob"],
        "write": [r"\bopen\s*\([^)]*['\"]w['\"]", r"write_text", r"write_bytes", r"os\.remove", r"shutil\.", r"mkdir", r"unlink"],
    },
    "network": {
        "patterns": [
            r"\brequests\.", r"\burllib\.", r"\bhttp\.client", r"\bsocket\.", r"\baiohttp", r"\bhttpx\.", r"\bwebsocket", r"fetch\s*\(", r"urllib\.request\.urlopen", r"socket\.create_connection"
        ]
    },
    "subprocess": {
        "patterns": [
            r"\bsubprocess\.", r"\bos\.system\s*\(", r"\bos\.popen\s*\(", r"\bexec\s*\(", r"\beval\s*\(", r"pty\.", r"multiprocessing"
        ]
    },
    "env": {
        "patterns": [r"os\.environ", r"os\.getenv", r"dotenv", r"load_dotenv"]
    },
    "secrets": {
        "patterns": [r"api_key", r"secret", r"token", r"password", r"credential"]
    }
}
