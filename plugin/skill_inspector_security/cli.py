...
def main():
    ...
    p_scan.add_argument("--previous", dest="previous", help="Previous snapshot path for diff (optional)", default=None)
    p_scan.add_argument("--port", type=int, default=8899, help="Port for report server (after scan)")
    p_scan.add_argument("--no-serve", action="store_true", help="Do not start report server after scan")
    p_scan.add_argument("--no-browser", action="store_true", help="Do not open browser")
    p_scan.add_argument("--runtime", action="store_true", help="Enable runtime behavior monitor (isolated execution)")
    p_scan.add_argument("--policy", dest="policy", help="Policy YAML file for enforcement", default=None)
    ...
    p_runtime = sub.add_parser("runtime", help="Runtime Behavior Monitor (isolated, collects filesystem/network/processes/env/DNS/outbound)")
    ...
    if args.cmd == "scan":
        ...
        result = scanner.scan(previous_snapshot=prev, enable_runtime=getattr(args, 'runtime', False), policy_path=pol)
        print(f"[Skill Inspector] Scan ID: {result['scan_id']}")
        print(f"[Skill Inspector] Skills: {result['summary']['total_skills']}  Findings: {result['summary']['total_findings']}  Avg Score: {result['summary']['avg_score']}")
        for s in result["skills"]:
            drift = len(s.get("drift", []))
            corr = len(s.get("correlation", []))
            rt = s.get("runtime_events", 0)
            print(f"  - {s['name']}: score {s['scorecard']['score']} ({s['scorecard']['grade']}) findings={len(s['findings'])} drift={drift} corr={corr} runtime={rt}")
        ...
        if result.get("policy_decision"):
            print(f"[Skill Inspector] Policy: {result['policy_decision']['decision']} violations={result['policy_decision']['count']}")
        ...
        if not args.no_serve:
            ...
            print(f"Run 'skill-inspector serve --port {args.port}' to view report at http://127.0.0.1:{args.port}")
        ...
        if not latest:
            ...
            sys.exit(1)
        print(f"[Skill Inspector] Serving latest scan {latest['scan_id']} on http://127.0.0.1:{args.port}")
        ...
        scanner = SecurityScanner(root)
        result = scanner.scan()
        print(json.dumps({"sbom_aggregate": result["summary"]["sbom_aggregate"], "skills": [{"name": s["name"], "sbom": s["sbom_path"]} for s in result["skills"]]}, indent=2))
        ...
        for e in graph.events[:10]:
            print(f"  - [{e.source}] {e.category}/{e.action} {e.subject} -> {e.target} ({e.capability})")
        if out:
            out.write_text(json.dumps([e.to_dict() for e in graph.events], indent=2), encoding="utf-8")
        ...
        pol = result.get("policy_decision") or evaluate_policy([f for s in result["skills"] for f in s["findings"]], load_policy(Path(args.policy)))
        ...
        import re
        forbidden = [r"requests\.post.*https?://", r"socket\.", r"urllib\.request\.urlopen", r"openai\.", r"boto3", r"telemetry", r"analytics"]
        # scan this project
        proj = Path(args.path)
        violations = []
        ...
        if violations:
            ...
            print("Offline compliance: PASS — no cloud services, no external API calls, loopback only")
