    ...
    for e in events:
                    ...
                    violations.append(
                        {
                            ...
                            "severity": "high",
                            "message": f"Network to {tgt} not in allow list {allow}",
                            "evidence": str(e),
                            "rule_id": "POL-NET",
                        }
                    )
                ...
                violations.append(
                    {
                        "policy": "network.deny",
                        "severity": "critical",
                        "message": f"Network to {tgt} denied by policy",
                        "evidence": str(e),
                        "rule_id": "POL-NET-DENY",
                    }
                )
        # subprocess
        if cat in ("processes", "subprocess") or e.get("capability") == "processes.execute":
            if not policy.get("subprocess", {}).get("allowed", False):
                violations.append(
                    {
                        "policy": "subprocess.allowed",
                        "severity": "high",
                        "message": f"Subprocess not allowed but observed: {tgt}",
                        "evidence": str(e),
                        "rule_id": "POL-PROC",
                    }
                )
                ...
                violations.append(
                    {
                        ...
                        "severity": "critical",
                        "message": f"Secrets handling not allowed: {rule}",
                        "evidence": str(e),
                        "rule_id": "POL-SEC",
                    }
                )
        # filesystem write
        if cat == "filesystem" and (
            "write" in str(e.get("capability", "")) or e.get("action") == "write"
        ):
                ...
                violations.append(
                    {
                        ...
                        "severity": "medium",
                        "message": f"Write to {tgt} not in allowed {allowed_writes}",
                        "evidence": str(e),
                        "rule_id": "POL-FS",
                    }
                )
