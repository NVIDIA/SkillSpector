...
def test_privacy():
    ...
    assert (
        "Financial" in priv["categories"]
        or "PII" in priv["categories"]
        or "Credentials" in priv["categories"]
    )
...
def test_offline_compliance():
    ...
    assert 'ALLOWED_BIND = "127.0.0.1"' in (
        proj / "skill_inspector_security" / "config.py"
