...
def test_local_only_finding_keeps_every_finding_on_the_same_path_local() -> None:
    """One provider-excluded finding blocks the shared file and one ledger work item."""
    eligible = _lineage_finding("eligible", "shared.py", 1)
    local = _lineage_finding("local", "shared.py", 2)
    local.tags.append("local-only")
    state: SkillspectorState = {
        "findings": [eligible, local],
        "use_llm": True,
        "llm_file_cache": {"shared.py": "must stay local"},
        "manifest": {},
        "model_config": {},
    }
    ...
    assert result["inspection_ledger"][0]["emitted_finding_ids"] == ["eligible", "local"]
    completeness, effective_ids = finalize_ledger(
        {
    ...
    assert completeness["execution_successful"] is True
    assert completeness["ledger_exceptions"] == []
    assert effective_ids == ["eligible", "local"]
...
def test_structural_hook_finding_never_constructs_llm_analyzer_or_gets_filtered() -> None:
    """LOW deterministic BH1 remains pass-through without relying on a local-only tag."""
    finding = Finding(
        rule_id="BH1",
        message="bundled hook",
        finding_id="bh1-structural",
        severity="LOW",
        confidence=0.1,
    ...
    state = {
        "findings": [finding],
        "file_cache": {"hooks/hooks.json": "raw-hook-canary"},
        "use_llm": True,
    }
    ...
    analyzer_cls.assert_not_called()
    assert [returned.finding_id for returned in result["findings"]] == ["bh1-structural"]
    assert result["effective_finding_ids"] == ["bh1-structural"]
    assert result["analyzer_status_events"][0]["status"] == "completed"


def test_structural_hook_finding_is_partitioned_before_llm_batching() -> None:
    """Mixed files send only ordinary findings to the provider and retain structural results."""
    structural = Finding(
        rule_id="BH1",
        message="bundled hook",
        finding_id="bh1-structural",
    ...
    ordinary_batch = Batch(file_path="ordinary.py", content="ordinary", findings=[ordinary])
    captured: list[Finding] = []

    def get_batches(_self, _files, _cache, findings):
        captured.extend(findings)
        return [ordinary_batch]

    with (
        patch(MOCK_PATCH_TARGET, _mock_get_chat_model),
        patch.object(LLMMetaAnalyzer, "get_batches", new=get_batches),
        patch.object(
            LLMMetaAnalyzer,
            "arun_batches",
            new_callable=AsyncMock,
            return_value=[(ordinary_batch, [])],
        ),
    ):
        result = meta_analyzer(
            {
                ...
                "file_cache": {
                    "hooks/hooks.json": "raw-hook-canary",
    ...
    assert [finding.finding_id for finding in captured] == ["ordinary"]
    assert [finding.finding_id for finding in result["findings"]] == [
        "bh1-structural",
        "ordinary",
    ]
    assert "llm-unconfirmed" in result["findings"][1].tags
    assert result["effective_finding_ids"] == ["bh1-structural", "ordinary"]


def test_structural_path_routes_all_same_file_findings_away_from_llm() -> None:
    """One hook finding keeps its raw activation document and companion findings local."""
    structural = Finding(
        rule_id="BH1",
        message="bundled hook",
        finding_id="bh1-structural",
    ...
    assert completeness["execution_successful"] is True
    assert effective_ids == ["bh1-structural", "ordinary-companion"]


def test_structural_lineage_stays_consistent_after_post_response_value_error() -> None:
    """Provider failure and deterministic rows agree on explicit effective-ID ordering."""
    structural = Finding(
        rule_id="BH1",
        message="bundled hook",
        finding_id="bh1-structural",
    ...
    completeness, effective_ids = finalize_ledger(
        {
            "components": ["hooks/hooks.json", "ordinary.py"],
            "findings": result["findings"],
            "effective_finding_ids": result["effective_finding_ids"],
            "inspection_ledger": result["inspection_ledger"],
            "analyzer_status_events": result["analyzer_status_events"],
    ...
    assert completeness["execution_successful"] is False
    assert not any(
        exception.get("reason_code") is LedgerReason.FINDING_ACCOUNTING_ERROR
        for exception in completeness["ledger_exceptions"]
    )
    assert effective_ids == ["bh1-structural", "ordinary"]


def test_no_llm_structural_finding_bypasses_confidence_filter() -> None:
    """Deterministic BH1 is retained below the ordinary no-LLM confidence threshold."""
    structural = Finding(
        rule_id="BH1",
        message="bundled hook",
        finding_id="bh1-structural",
    ...
    result = meta_analyzer({"findings": [structural], "use_llm": False})
