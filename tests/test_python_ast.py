...
@pytest.mark.parametrize(
    ("path", "launcher"),
    [
        pytest.param("run", "/usr/bin/env -S uv", id="env-command-name"),
        pytest.param("run", "/usr/bin/env uv", id="env-opaque-command-name"),
        pytest.param("nested/run", "/usr/bin/env -S uv", id="env-command-basename"),
        pytest.param("nested/RUN", "/usr/bin/env -S uv", id="darwin-command-alias"),
        pytest.param("--offline", "/usr/bin/env -S uv", id="env-global-option"),
        pytest.param("run", "/usr/local/bin/uv", id="direct-command-name"),
        pytest.param("-q", "/usr/local/bin/uv", id="direct-global-option"),
    ],
)
def test_bare_uv_launcher_with_runtime_selectable_source_is_ambiguous(
    path: str, launcher: str
) -> None:
    content = f"#!{launcher}\npass\n"

    assert classify_python_source(path, content) is PythonSourceClassification.AMBIGUOUS


def test_bare_uv_launcher_with_inert_source_name_is_non_python() -> None:
    content = "#!/usr/bin/env -S uv\npass\n"
...
@pytest.mark.parametrize("launcher_kind", ["env", "direct"])
@pytest.mark.parametrize(
    ("source_name", "caller_arguments"),
    [
        pytest.param("run", ("--script", "run"), id="command-name"),
        pytest.param(
            "--offline",
            ("run", "--script", "./--offline"),
            id="global-option",
...
def test_real_bare_uv_launcher_can_select_source(
    tmp_path: Path,
    launcher_kind: str,
    source_name: str,
    ...
    if launcher_kind == "direct" and any(character.isspace() for character in uv_path):
        pytest.skip("direct uv shebang path contains whitespace")
    launcher = "/usr/bin/env -S uv" if launcher_kind == "env" else uv_path
    ...
    assert executed.returncode == 0, executed.stderr
    assert marker in executed.stdout
    assert classify_python_source(source.name, content) is PythonSourceClassification.AMBIGUOUS
